		...
		.transform("t" + rightLaurelX + "," + rightLaurelY)
		.attr("fill", fillColor);
		...
		pointerLeaf.transform("T" + (insertPoint.x + rightLaurelX - theBBox.x - 3) + "," + (insertPoint.y + rightLaurelY - theBBox.y - 1)
			+ "r-185" + "," + (theBBox.x) + "," + (theBBox.y));
		rightLaurelGroup.append(pointerLeaf);
		adjustMarkers(laurelsRight,9,leafOutlines, rightLaurelX, rightLaurelY);

		rightLaurelGroup.transform("s1.25");

		var leftLaurelGroup  = rightLaurelGroup.clone();
		leftLaurelGroup.transform("T" + leftLaurelX + "," + 0 + "s-1.25,1.25");
...
function adjustMarkers(thePath, spacing, theSymbol, xAdjust, yAdjust){
	...
	for (var i = 0; i < spacing; i++){
		...
		rightLaurelGroup.append(leaves);
		...
		rightLaurelGroup.append(oppLeaves);
	}
